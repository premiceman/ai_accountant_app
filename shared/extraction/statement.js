...
function normaliseCategory(raw) {
    ...
    if (CATEGORY_CANONICAL.has(key))
        return CATEGORY_CANONICAL.get(key);
    for (const [alias, canonical] of CATEGORY_CANONICAL.entries()) {
        if (key.includes(alias))
            return canonical;
    }
    const normalized = key.replace(/[^a-z]+/g, ' ').trim();
    if (CATEGORY_CANONICAL.has(normalized))
        return CATEGORY_CANONICAL.get(normalized);
    if (CATEGORY_LIST.includes(capitalize(raw)))
...
function summariseCategories(transactions) {
    ...
    for (const tx of transactions) {
        ...
        if (tx.direction === 'inflow')
            ...
            item.outflow += Math.abs(tx.amount);
        groups.set(key, item);
    }
...
async function llmCategoriseTransactions(transactions) {
    ...
    updates.forEach((item) => {
        ...
        if (!Number.isInteger(idx) || idx < 0 || idx >= transactions.length)
            return;
...
function heuristicStatementParsing(text) {
    ...
    for (const line of lines) {
        ...
        if (directionToken === 'dr' && amount > 0)
            amount = -amount;
        if (directionToken === 'cr' && amount < 0)
        ...
        let category = 'Other';
        if (/tesco|waitrose|sainsbury|aldi|lidl|asda/.test(lower))
            category = 'Groceries';
        else if (/uber|trainline|tfl|rail|hotel|airbnb/.test(lower))
            category = 'Travel';
        else if (/shell|bp|esso|petrol|fuel/.test(lower))
            category = 'Car expenses';
        else if (/netflix|spotify|disney|prime|entertain/.test(lower))
            category = 'Entertainment';
        else if (/restaurant|cafe|coffee|food|eat|dining/.test(lower))
            category = 'Meals Out';
        else if (/rent|mortgage|landlord/.test(lower))
            category = 'Rent/Mortgage';
        else if (/transfer|to savings|isa|standing order/.test(lower))
            category = 'Transfers';
        transactions.push({
