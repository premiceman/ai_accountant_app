...
async function extractPdfText(buffer: Buffer | null | undefined): Promise<string> {
...
export async function extractStatement(
  ...
  const startIso =
    (metadata.period?.start ? new Date(metadata.period.start).toISOString().slice(0, 10) : null) ||
    (derivedPeriod.start ? new Date(derivedPeriod.start).toISOString().slice(0, 10) : null) ||
    new Date().toISOString().slice(0, 10);
  const endIso =
    (metadata.period?.end ? new Date(metadata.period.end).toISOString().slice(0, 10) : null) ||
...
export { analyseCurrentAccountStatement, normaliseCategory, summariseStatement };
